...
import { HOME_PATH, KCM_DIR, KCM_PATH, kmcPath, KUBE_PATH, KCM_CONFIG_PATH, KCM_FILE,exec } from './utils'

const pkg: any = PKG

updateNotifier({pkg}).notify()

if (!shell.test('-d', path.join(HOME_PATH, KCM_DIR))) {
    shell.mkdir(path.join(HOME_PATH, KCM_DIR))
}
...
program
    ...
    .action(function (name) {
        ...
        let files = shell.ls(KUBE_PATH)
            .map(f => f)

        if (files.length > 0) {
            console.log(`Warning: '.kube' is not empty, making a backup in ${kmcPath('_backup')}`)
            shell.cp('-R', path.resolve(KUBE_PATH,'*'), kmcPath('_backup'))
        }
